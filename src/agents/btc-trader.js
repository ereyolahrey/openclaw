...
class BTCTraderAgent {
  ...
  async runCycle() {
    ...
    // 6a. Minimum confidence — skip coin-flip trades
    if (analysis.confidence < 0.30) {
      this.log.info(`⚠️ Low confidence (${(analysis.confidence * 100).toFixed(0)}% < 30%) — sitting out.`);
      direction = "HOLD";
    }

    // 6b. MUST agree with Bayse market — never fight the crowd
    if (direction !== "HOLD" && !analysis.agreesWithMarket) {
      this.log.info(`⚠️ Our signal (${direction}) DISAGREES with Bayse market (${analysis.signals.market}) — sitting out.`);
      direction = "HOLD";
    }

    // 6c. Spike/mean-reversion guard — if price just spiked, high reversal risk
    if (direction !== "HOLD" && analysis.latest.spikeDetected) {
      this.log.info(`⚠️ Spike detected — mean reversion risk, sitting out.`);
      direction = "HOLD";
    }
    ...
    // 7. Share price filter — skip if > 0.65 (poor payout after fees)
    ...
    if (sharePrice > 0.65) {
      this.log.info(`Share price ${sharePrice.toFixed(2)} > 0.65 — poor value, skipping.`);
      this.analyzedEventIds.add(event.id);
      await this.notify(`⚠️ Skipped: ${this._esc(outcomeName)} shares at ${(sharePrice * 100).toFixed(0)}c — too expensive (max 65c)\nEvent: ${this._esc(event.title)}`);
      return;
    }

    // 8. V2 gap strength filter — gap must be at least 0.3x typical move to be meaningful
    if (analysis.latest.gapStrength < 0.3) {
      this.log.info(`Gap strength ${analysis.latest.gapStrength.toFixed(2)}x < 0.3x typical move — too close to call, skipping.`);
      this.analyzedEventIds.add(event.id);
      await this.notify(`⚠️ Skipped: gap only ${analysis.latest.gapStrength.toFixed(2)}x typical move — too close\nEvent: ${this._esc(event.title)}`);
      return;
    }
